...
def generate_full_export_excel(campaign: Campaign) -> HttpResponse:
    ...
    companies = (
        CompanyNumber.objects.filter(campaign=campaign)
        .select_related(
            "house_data",
            "vat_lookup",
            "website_hunting_result",
            "website_contact_lookup",
            "linkedin_lookup",
            "snov_lookup",
            "hunter_lookup",
        )
    ...
    # Create Final Result sheet (first sheet)
    create_final_result_sheet(workbook, companies)
    ...
    filename = (
        f"{campaign.name}-full-export-{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    )
...
def create_final_result_sheet(workbook: Workbook, companies) -> None:
    """
    Create Final Result sheet with consolidated data from all enrichment steps.
    ...
    """
    # Create worksheet
    sheet = workbook.create_sheet("Final Result")
    ...
    headers = [
        ...
        "Company Name",
        "VAT Number",
        "Website_Hunting Resulting Domains",
        "Website_Hunting Approved Domain",
        "Website_Contact Extracted Emails",
        "Website_Contact Extracted Phones",
        "Website Contact Extracted Linkedin URLs",
        "Linkedin_Discovery Company URLs",
        "Linkedin_Discovery Employee URLs",
        "SNOV_Email_Extraction Extracted Emails",
        "Hunter_Domain_Search Found Emails",
        "Total Emails Found",
        "All emails - Deduplicate",
    ]
    ...
    for col_num, header in enumerate(headers, 1):
        ...
        cell.fill = PatternFill(
            start_color="E6F3E6", end_color="E6F3E6", fill_type="solid"
        )
        cell.alignment = Alignment(horizontal="center")

    # Add data rows - only for companies with emails
    row_num = 2
    serial_number = 1
    ...
    for company in companies:
        ...
        # Process Website Hunting Resulting Domains
        hunting_domains = ""
        if hunting_data and hunting_data.ranked_domains:
            if isinstance(hunting_data.ranked_domains, list):
                ...
                for domain_data in hunting_data.ranked_domains:
                    if isinstance(domain_data, dict):
                        ...
                        domain_parts.append(f"{domain} ({score})")
                    else:
                        domain_parts.append(str(domain_data))
                hunting_domains = "; ".join(domain_parts)
            else:
                hunting_domains = str(hunting_data.ranked_domains)

        # Process Website Contact Extracted Emails
        contact_emails = ""
        contact_email_count = 0
        if contact_data and contact_data.email_addresses:
            if isinstance(contact_data.email_addresses, list):
                contact_emails = "; ".join(contact_data.email_addresses)
                contact_email_count = len(contact_data.email_addresses)
            else:
                contact_emails = str(contact_data.email_addresses)
                contact_email_count = 1

        # Process Website Contact Extracted Phones
        contact_phones = ""
        if contact_data and contact_data.phone_numbers:
            if isinstance(contact_data.phone_numbers, list):
                contact_phones = "; ".join(contact_data.phone_numbers)
            else:
                contact_phones = str(contact_data.phone_numbers)

        # Process Website Contact Extracted LinkedIn URLs
        contact_linkedin_urls = ""
        if contact_data and contact_data.social_media_links:
            linkedin_links = contact_data.social_media_links.get("linkedin", [])
            if isinstance(linkedin_links, list) and linkedin_links:
                contact_linkedin_urls = "; ".join(linkedin_links)
            elif isinstance(linkedin_links, str) and linkedin_links.strip():
                contact_linkedin_urls = linkedin_links

        # Process LinkedIn Discovery Company URLs
        linkedin_company_urls = ""
        if linkedin_data and linkedin_data.company_urls:
            if isinstance(linkedin_data.company_urls, list):
                company_url_parts = []
                for company_profile in linkedin_data.company_urls:
                    if isinstance(company_profile, dict):
                        ...
                        company_url_parts.append(str(company_profile))
                linkedin_company_urls = "; ".join(company_url_parts)
            else:
                linkedin_company_urls = str(linkedin_data.company_urls)

        # Process LinkedIn Discovery Employee URLs
        linkedin_employee_urls = ""
        if linkedin_data and linkedin_data.employee_urls:
            if isinstance(linkedin_data.employee_urls, list):
                employee_url_parts = []
                for employee_profile in linkedin_data.employee_urls:
                    if isinstance(employee_profile, dict):
                        ...
                        employee_url_parts.append(str(employee_profile))
                linkedin_employee_urls = "; ".join(employee_url_parts)
            else:
                linkedin_employee_urls = str(linkedin_data.employee_urls)

        # Process SNOV Email Extraction
        snov_emails = ""
        snov_email_count = 0
        if snov_data and snov_data.emails_found:
            if isinstance(snov_data.emails_found, list):
                email_parts = []
                for email_data in snov_data.emails_found:
                    if isinstance(email_data, dict):
                        email = email_data.get("email", "Unknown")
                        email_parts.append(email)
                    else:
                        email_parts.append(str(email_data))
                snov_emails = "; ".join(email_parts)
                snov_email_count = len(email_parts)
            else:
                snov_emails = str(snov_data.emails_found)
                snov_email_count = 1

        # Process Hunter Domain Search
        hunter_emails = ""
        hunter_email_count = 0
        if hunter_data and hunter_data.emails_found:
            if isinstance(hunter_data.emails_found, list):
                email_parts = []
                for email_data in hunter_data.emails_found:
                    if isinstance(email_data, dict):
                        ...
                        email_parts.append(str(email_data))
                hunter_emails = "; ".join(email_parts)
                hunter_email_count = len(email_parts)
            else:
                hunter_emails = str(hunter_data.emails_found)
                hunter_email_count = 1

        # Collect and deduplicate all emails from all sources
        all_emails_set = set()

        # Add emails from contact data
        if contact_data and contact_data.email_addresses:
            if isinstance(contact_data.email_addresses, list):
                all_emails_set.update(contact_data.email_addresses)
            else:
                all_emails_set.add(str(contact_data.email_addresses))

        # Add emails from SNOV data
        if snov_data and snov_data.emails_found:
            if isinstance(snov_data.emails_found, list):
                for email_data in snov_data.emails_found:
                    if isinstance(email_data, dict):
                        ...
                        all_emails_set.add(str(email_data))
            else:
                all_emails_set.add(str(snov_data.emails_found))

        # Add emails from Hunter data
        if hunter_data and hunter_data.emails_found:
            if isinstance(hunter_data.emails_found, list):
                for email_data in hunter_data.emails_found:
                    if isinstance(email_data, dict):
                ...
                all_emails_set.add(str(hunter_data.emails_found))

        # Remove empty strings and None values
        all_emails_set.discard("")
        all_emails_set.discard(None)
        all_emails_set.discard("None")
        all_emails_set.discard("Unknown")

        # Only include this row if there are emails
        if len(all_emails_set) == 0:
            continue

        # Convert to sorted list for consistent output
        all_emails_list = sorted(list(all_emails_set))
        all_emails_deduplicated = "; ".join(all_emails_list)

        # Calculate Total Emails Found
        total_emails = contact_email_count + snov_email_count + hunter_email_count

        # Prepare row data
        row_data = [
            serial_number,  # Serial number (Sr)
        ...
        # Increment counters for next row
        row_num += 1
        serial_number += 1
...
def create_companies_house_sheet(workbook: Workbook, companies) -> None:
    ...
    for row_num, company in enumerate(companies, 2):
        ...
        row_data = [
            ...
            getattr(house_data, "confirmation_statement_date", "")
            if house_data
            else "",
            getattr(house_data, "confirmation_statement_next_due", "")
            if house_data
            else "",
            getattr(house_data, "confirmation_statement_overdue", "")
            if house_data
            else "",
            ...
            getattr(house_data, "status", "NOT_PROCESSED")
            if house_data
            else "NOT_PROCESSED",
            house_data.created_at.strftime("%Y-%m-%d %H:%M:%S")
            if house_data and house_data.created_at
            else "",
        ]
...
def create_vat_lookup_sheet(workbook: Workbook, companies) -> None:
    ...
    for row_num, company in enumerate(companies, 2):
        ...
        row_data = [
            ...
            getattr(vat_lookup, "status", "NOT_PROCESSED")
            if vat_lookup
            else "NOT_PROCESSED",
            getattr(vat_lookup, "processing_notes", "") if vat_lookup else "",
            getattr(vat_lookup, "proxy_used", "") if vat_lookup else "",
            vat_lookup.created_at.strftime("%Y-%m-%d %H:%M:%S")
            if vat_lookup and vat_lookup.created_at
            else "",
        ]
    ...
    for row_num, company in enumerate(companies, 2):
        ...
        if hunting_data:
            ...
            if hunting_data.domains_found:
                ...
                domains_found_str = (
                    "; ".join(hunting_data.domains_found)
                    if isinstance(hunting_data.domains_found, list)
                    else str(hunting_data.domains_found)
                )
        ...
        row_data = [
            ...
            hunting_data.created_at.strftime("%Y-%m-%d %H:%M:%S")
            if hunting_data and hunting_data.created_at
            else "",
        ]
...
def create_website_contact_extraction_sheet(workbook: Workbook, companies) -> None:
    ...
    for col_num, header in enumerate(headers, 1):
        ...
        cell.font = Font(bold=True)
        cell.fill = PatternFill(
            start_color="E0F8F8", end_color="E0F8F8", fill_type="solid"
        )
        cell.alignment = Alignment(horizontal="center")
    ...
    for row_num, company in enumerate(companies, 2):
        ...
        if contact_data:
            ...
            if contact_data.phone_numbers and isinstance(
                contact_data.phone_numbers, list
            ):
                phone_count = len(contact_data.phone_numbers)
                phone_numbers_str = "; ".join(contact_data.phone_numbers)

            # Handle email addresses
            if contact_data.email_addresses and isinstance(
                contact_data.email_addresses, list
            ):
                email_count = len(contact_data.email_addresses)
                email_addresses_str = "; ".join(contact_data.email_addresses)

            # Handle social media links
            if contact_data.social_media_links and isinstance(
                contact_data.social_media_links, dict
            ):
                social_media_parts = []
        ...
        row_data = [
            ...
            getattr(contact_data, "status", "NOT_PROCESSED")
            if contact_data
            else "NOT_PROCESSED",
            getattr(contact_data, "processing_notes", "") if contact_data else "",
            contact_data.created_at.strftime("%Y-%m-%d %H:%M:%S")
            if contact_data and contact_data.created_at
            else "",
        ]
...
def create_linkedin_profile_discovery_sheet(workbook: Workbook, companies) -> None:
    ...
    for row_num, company in enumerate(companies, 2):
        ...
        if linkedin_data:
            # Handle company URLs
            if linkedin_data.company_urls and isinstance(
                linkedin_data.company_urls, list
            ):
            ...
            if linkedin_data.employee_urls and isinstance(
                linkedin_data.employee_urls, list
            ):
        ...
        row_data = [
            ...
            getattr(linkedin_data, "search_status", "NOT_PROCESSED")
            if linkedin_data
            else "NOT_PROCESSED",
            ...
            linkedin_data.created_at.strftime("%Y-%m-%d %H:%M:%S")
            if linkedin_data and linkedin_data.created_at
            else "",
        ]
...
def get_full_export_summary(campaign: Campaign) -> dict:
    ...
    """
    companies = CompanyNumber.objects.filter(campaign=campaign).select_related(
        "house_data",
        "vat_lookup",
        "website_hunting_result",
        "website_contact_lookup",
    ...
    return {
        ...
        "total_linkedin_profiles_found": total_company_profiles_found
        + total_employee_profiles_found,
        "completion_percentage": round((with_house_data / total_companies) * 100, 1)
