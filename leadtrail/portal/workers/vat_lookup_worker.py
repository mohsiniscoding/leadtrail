...
A standalone worker script using the schedule library to process VAT lookups
for companies that have completed Companies House processing.
...
from leadtrail.portal.models import CompanyNumber, VATLookup
...
def run_vat_lookup():
    """
    Main worker function to perform VAT lookup for companies that have completed Companies House processing.
    ...
    logger.info(f"Starting VAT lookup (batch size: {batch_size})")
    ...
    try:
        ...
        error_msg = f"VAT lookup failed: {str(e)}"
...
def main():
    ...
    logger.info("Starting VAT Lookup Worker")
    ...
    schedule.every(10).seconds.do(run_vat_lookup)
    
    # Run immediately on startup
    logger.info("Running initial VAT lookup...")
    run_vat_lookup()
    ...
    logger.info("VAT Lookup Worker stopped")
