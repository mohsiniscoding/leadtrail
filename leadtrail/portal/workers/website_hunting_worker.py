#!/usr/bin/env python
"""
Website Hunting Worker
=====================

A standalone worker script using the schedule library to process website hunting
operations by combining SERP search for domain discovery and website crawling 
for domain ranking.
...
Schedule:
    Runs every 3 minutes (matching original Celery schedule)
"""
...
def run_website_hunting():
    """
    Main worker function to perform website hunting for companies that have completed VAT lookup.
    
    Processes companies using a two-phase workflow: SERP discovery + website crawling.
    ...
    try:
        ...
        error_msg = f"Website hunting failed: {str(e)}"
...
def main():
    ...
    logger.info("Starting Website Hunting Worker")
    ...
    logger.info("Website Hunting Worker stopped")
